...
<head>
  ...
  <style>
    ...
    *{{box-sizing:border-box}} html{{color-scheme:dark}} body{{margin:0;min-height:100vh;background:radial-gradient(circle at 50% -20%,#102746 0,transparent 35%),var(--bg);color:var(--text);font-family:"Segoe UI Variable Text","Segoe UI Variable","Segoe UI",Inter,system-ui,-apple-system,sans-serif;font-size:16px;line-height:1.6;text-rendering:optimizeLegibility;-webkit-font-smoothing:antialiased;font-variant-numeric:tabular-nums}}
    ...
    @media(max-width:560px){{.market-page{{width:min(100% - 24px,1180px)}}.detail-section{{padding:19px}}.technical-heading,.outlook-heading,.fundamental-heading,.catalyst-heading,.trader-brief-heading,.follow-heading{{align-items:flex-start;flex-direction:column}}.technical-heading span{{white-space:normal}}.technical-grid{{grid-template-columns:1fr}}.technical-structure{{grid-column:auto}}.technical-outlook{{padding:16px}}.rule-group{{padding:15px}}.rule-copy>div{{flex-direction:column;gap:2px}}.fundamental-row{{grid-template-columns:1fr 1fr}}.fundamental-row>div:first-child{{grid-column:1/-1}}.fundamental-row>a{{text-align:left}}.catalyst-row,.brief-columns,.follow-row{{grid-template-columns:1fr}}.change-row{{grid-template-columns:1fr auto 20px auto}}.scan-trail-head{{display:none}}.scan-trail-row{{grid-template-columns:1fr 1fr}}}}
  </style>
</head>
<body>
