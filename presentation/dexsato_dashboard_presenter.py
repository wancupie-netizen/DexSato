  ...
  <script>
    ...
    if(chartRoot){{
      ...
      async function loadLiveQuote(){{
        ...
        try{{const response=await fetch("/api/markets/"+encodeURIComponent(chartRoot.dataset.token)+"/quote",{{cache:"no-store"}});if(!response.ok)throw new Error("Quote request failed");const data=await response.json(),formatted=formatLivePrice(data.price_usd);if(!formatted)throw new Error("Invalid quote");livePrice.textContent=formatted;if(Number.isFinite(Number(data.price_change_24h))){{const change=Number(data.price_change_24h);liveChange.textContent=(change>=0?"+":"")+change.toFixed(2)+"% · 24h";}}setQuoteStatus(data.status);quoteUpdated.textContent=(data.status==="STALE"?"Last verified exact-pool price · ":"Exact-pool price · ")+formatMYT(data.as_of)+" · "+(data.source||"DexScreener");}}
