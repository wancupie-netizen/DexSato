...
<head>
  ...
  <style>
    ...
    *{{box-sizing:border-box}} html{{color-scheme:dark;scroll-behavior:smooth}} body{{margin:0;background:radial-gradient(circle at 50% -15%,#102746 0,transparent 34%),var(--bg);color:var(--text);font-family:"Segoe UI Variable Text","Segoe UI Variable","Segoe UI",Inter,system-ui,sans-serif;font-size:16px;line-height:1.55;text-rendering:optimizeLegibility;-webkit-font-smoothing:antialiased;font-variant-numeric:tabular-nums}}
    ...
    .top-actions,.theme-switcher{{display:flex;align-items:center;gap:8px}} .theme-switcher{{gap:3px;padding:3px;border:1px solid var(--line);border-radius:10px;background:var(--panel)}} .theme-option{{padding:7px 10px;border:0;border-radius:7px;background:transparent;color:var(--muted);font-size:12px;font-weight:800;cursor:pointer}} .theme-option.active{{background:#1b1d4e;color:#fff}} .learn-link{{padding:9px 12px;border:1px solid var(--line);border-radius:9px;color:var(--text);font-size:13px;font-weight:750;text-decoration:none}}
    ...
    @media(max-width:430px){{
      .shell{{width:calc(100% - 20px)}} .brand span,.learn-link{{display:none}} .brand img{{width:135px}}
...
<body><main class="shell">
  ...
  const themeOptions=[...document.querySelectorAll("[data-theme-option]")];function applyTheme(theme){{const value=theme==="plain"?"plain":"current";if(value==="plain")document.documentElement.dataset.theme="plain";else delete document.documentElement.dataset.theme;themeOptions.forEach(button=>button.classList.toggle("active",button.dataset.themeOption===value));try{{localStorage.setItem("dexsato-theme",value);}}catch(error){{}}}}let saved="current";try{{saved=localStorage.getItem("dexsato-theme")||"current";}}catch(error){{}}applyTheme(saved);themeOptions.forEach(button=>button.addEventListener("click",()=>applyTheme(button.dataset.themeOption)));
