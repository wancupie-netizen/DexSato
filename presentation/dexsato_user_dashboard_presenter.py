...
<head>
  ...
  <style>
    ...
    *{{box-sizing:border-box}} html{{color-scheme:dark;scroll-behavior:smooth}} body{{margin:0;background:radial-gradient(circle at 50% -15%,#102746 0,transparent 34%),var(--bg);color:var(--text);font-family:"Segoe UI Variable Text","Segoe UI Variable","Segoe UI",Inter,system-ui,sans-serif;font-size:16px;line-height:1.55;text-rendering:optimizeLegibility;-webkit-font-smoothing:antialiased;font-variant-numeric:tabular-nums}}
    ...
    .brand{{display:flex;align-items:baseline;gap:8px;white-space:nowrap}}.brand img{{display:none}}.brand strong{{font-family:"Space Grotesk",sans-serif;font-size:19px;letter-spacing:.01em}}.brand span{{font-family:"JetBrains Mono",monospace;color:#45505F;font-size:10px;letter-spacing:.08em}}
    .header-search{{flex:1;max-width:420px;height:36px;border:1px solid var(--line);background:var(--panel2);display:flex;align-items:center;gap:8px;padding:0 12px;color:var(--muted)}}.header-search svg{{width:14px;height:14px;flex:0 0 auto}}.header-search input{{width:100%;border:0;outline:0;background:transparent;color:var(--text-dim);font:500 12.5px "JetBrains Mono",monospace}}.header-search input::placeholder{{color:var(--text-faint)}}.header-search kbd{{margin-left:auto;border:1px solid var(--line-bright,#2A3847);padding:2px 5px;color:var(--text-faint);font:500 10px "JetBrains Mono",monospace}}
    .top-actions{{margin-left:auto;display:flex;align-items:center;gap:14px}}.gas-pill{{height:32px;border:1px solid var(--line);padding:0 10px;display:flex;align-items:center;gap:7px;color:var(--text-dim);font:500 11px "JetBrains Mono",monospace;white-space:nowrap}}.gas-pill .gas-dot{{width:6px;height:6px;background:var(--cyan);box-shadow:0 0 6px var(--cyan)}}.connect-wallet-btn{{height:36px;padding:0 16px;border:0;background:var(--cyan);color:#06110F;font:600 12.5px "Space Grotesk",sans-serif;cursor:default;clip-path:polygon(8px 0,100% 0,100% calc(100% - 8px),calc(100% - 8px) 100%,0 100%,0 8px)}}
    ...
    @media(max-width:980px){{.header-search{{max-width:300px}}.gas-pill{{display:none}}}}
...
<body><div class="app-shell">
  ...
  <div class="app-main">
    <header class="topbar"><div class="brand"><strong>dexsato</strong><span>DEX INTELLIGENCE</span></div><label class="header-search" aria-label="Search token, pair or contract"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4.3-4.3"/></svg><input type="search" placeholder="Search token, pair or contract" autocomplete="off"><kbd>/</kbd></label><div class="top-actions"><div class="gas-pill" aria-label="Gas status"><span class="gas-dot"></span><span>Gas · —</span></div><button class="connect-wallet-btn" type="button" aria-label="Connect Wallet">Connect Wallet</button><a class="learn-link" href="#how-to-read">How to read</a><div class="theme-switcher" role="group" aria-label="Dashboard theme"><button class="theme-option active" type="button" data-theme-option="current" aria-label="Use current dark theme" title="Current dark theme" aria-pressed="true">●</button><button class="theme-option" type="button" data-theme-option="plain" aria-label="Use plain theme" title="Plain theme" aria-pressed="false">○</button></div></div></header>
    <main class="shell">
