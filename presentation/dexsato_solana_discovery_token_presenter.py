...
<aside><section class="card jupiter" data-jupiter-sandbox data-token-address="__TOKEN__"><span class="eyebrow">Jupiter integration sandbox</span><h3>Read-only quote</h3><span class="badge">D5 · NO EXECUTION</span><p class="sandbox-note">Compare a Jupiter route with the market observation above. No transaction is built, signed or submitted.</p><div class="wallet-state" data-wallet-state>Wallet not connected · optional for quote</div><div class="sandbox-form"><button class="sandbox-button" type="button" data-connect-wallet>Connect supported wallet</button><label for="jupiter-amount">Amount in SOL</label><input class="sandbox-input" id="jupiter-amount" data-quote-amount inputmode="decimal" type="number" min="0.001" max="100" step="0.001" value="0.1"><button class="sandbox-button primary" type="button" data-get-quote>Get Jupiter quote</button></div><div class="quote-result" data-quote-result aria-live="polite"></div><p class="quote-policy">DexSato integrator fee: <strong>0 bps</strong>. DexSato never asks for a seed phrase and does not hold keys or funds.</p></section></aside></div>
<footer><span>Experimental discovery · evidence synthesis only · not financial advice.</span><span>Market observations and Jupiter quotes are distinct.</span></footer></main><script>document.querySelectorAll("[data-copy-address]").forEach(function(button){button.addEventListener("click",async function(){try{await navigator.clipboard.writeText(button.dataset.copyAddress);button.textContent="Copied";window.setTimeout(function(){button.textContent="Copy"},1600)}catch(error){button.textContent="Unavailable"}})});(function(){const sandbox=document.querySelector("[data-jupiter-sandbox]");if(!sandbox)return;const walletState=sandbox.querySelector("[data-wallet-state]");const connect=sandbox.querySelector("[data-connect-wallet]");const quoteButton=sandbox.querySelector("[data-get-quote]");const amount=sandbox.querySelector("[data-quote-amount]");const result=sandbox.querySelector("[data-quote-result]");function text(value,fallback){return value===null||value===undefined||value===""?fallback:String(value)}connect.addEventListener("click",async function(){const provider=window.phantom&&window.phantom.solana?window.phantom.solana:window.solana;if(!provider||typeof provider.connect!=="function"){walletState.textContent="Supported Solana wallet not detected.";return}connect.disabled=true;try{const connection=await provider.connect();const key=connection&&connection.publicKey?connection.publicKey:provider.publicKey;walletState.textContent="Connected public key · "+text(key,"Unavailable");connect.textContent="Wallet connected"}catch(error){walletState.textContent="Wallet connection was not approved."}finally{connect.disabled=false}});quoteButton.addEventListener("click",async function(){quoteButton.disabled=true;quoteButton.textContent="Fetching quote…";result.className="quote-result visible";result.textContent="Requesting a read-only Jupiter quote…";try{const url="/api/discovery/solana/"+encodeURIComponent(sandbox.dataset.tokenAddress)+"/jupiter-quote?amount_sol="+encodeURIComponent(amount.value);const response=await fetch(url,{headers:{accept:"application/json"},credentials:"same-origin"});const payload=await response.json();if(!response.ok)throw new Error(payload.detail||"Quote unavailable");const output=payload.output_amount_ui?payload.output_amount_ui+" __SYMBOL__":payload.output_amount_raw+" raw token units";result.innerHTML='<div class="quote-grid"><div class="quote-cell"><span>Expected output</span><b>'+text(output,"Unavailable")+'</b></div><div class="quote-cell"><span>Router</span><b>'+text(payload.router,"Jupiter")+'</b></div><div class="quote-cell"><span>Price impact</span><b>'+text(payload.price_impact_pct,"Unavailable")+(payload.price_impact_pct===null||payload.price_impact_pct===undefined?"":"%")+'</b></div><div class="quote-cell"><span>Quote mode</span><b>'+text(payload.mode,"ExactIn")+'</b></div></div><div class="fee-row"><span>Jupiter / route fee</span><b>'+text(payload.jupiter_fee_bps,0)+' bps</b></div><div class="fee-row"><span>DexSato integrator fee</span><b>'+text(payload.dexsato_integrator_fee_bps,0)+' bps</b></div><p class="quote-policy">Quote only · no transaction material returned · '+text(payload.as_of,"Time unavailable")+'</p>'}catch(error){result.innerHTML='<strong class="quote-error">'+text(error.message,"Jupiter quote is unavailable.")+'</strong>'}finally{quoteButton.disabled=false;quoteButton.textContent="Get Jupiter quote"}})})()</script></body></html>"""
    replacements = {
