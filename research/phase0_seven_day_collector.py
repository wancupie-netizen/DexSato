#!/usr/bin/env python3
"""DexSato Phase 0 seven-day Solana discovery collector.

Runs once per invocation. Windows Task Scheduler should invoke it every 15
minutes. It is intentionally isolated from the production engine, dashboard,
database, Telegram notifier, and public application.
"""
...
def experiment_status(state: dict[str, Any], current: datetime) -> str:
    ends = parse_time(state["ends_at"]) or current
    if current >= ends or state["run_count"] >= MAX_RUNS:
        return "COMPLETE"
...
<title>DexSato Phase 0 Monitor</title>
<style>
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one scheduled DexSato Phase 0 collection cycle.")
    ...
    output = discovery_storage_dir(DEFAULT_OUTPUT) if args.output_dir is None else Path(args.output_dir)
    ...
    try:
        ...
        status_before = experiment_status(state, current)
        if status_before == "COMPLETE":
            summary = {"collector_status": "COMPLETE", "metrics": metrics(state, current)}
            ...
            print("Phase 0 seven-day experiment is complete; no API calls were made.")
            return 0
        ...
        write_latest_run(output / "latest-run.txt", state, summary)
        write_html(output / "status.html", state, summary)
        print(f"Phase 0 run {state['run_count']}/{MAX_RUNS}: {summary['collector_status']}")
